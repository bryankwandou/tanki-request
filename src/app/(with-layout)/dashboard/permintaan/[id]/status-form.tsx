...
export function StatusForm({
  ...
  return (
    <form action={formAction} className="space-y-3">
      ...
      {/*
        Penanda publik/internal (Issue #6). Default tercentang supaya alasan
        penolakan tetap sampai ke pelapor seperti sekarang; operator yang menulis
        catatan internal cukup melepas centangnya.
      */}
      <label className="flex items-start gap-2 text-sm text-dark-5 dark:text-dark-6">
        <input
          type="checkbox"
          name="catatanPublik"
          defaultChecked
          className="mt-0.5 h-4 w-4 rounded border-stroke dark:border-dark-3"
        />
        <span>
          Tampilkan catatan ini kepada pelanggan
          <span className="block text-xs opacity-75">
            Bila dilepas, catatan hanya terlihat operator — tidak muncul di /lacak
            dan tidak ikut dikirim lewat email.
          </span>
        </span>
      </label>
